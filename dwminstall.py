...
def main():
    parse = argparse.ArgumentParser(description="Download and installs dwm a suckless window manager.")
    parse.add_argument("-b", "--background", 
        metavar="PATH", 
        help="File path of image to add as a background", 
        type=str
    )
    parse.add_argument("-d", "--default-dwm", 
        action="store_true", 
        help="If flag is set, default dwm will be installed instead of a custom dwm"
    )
    parse.add_argument("-r", "--reboot", 
        action="store_true", 
    ...
    parse.add_argument("-p", "--powersaver", 
        action="store_false", 
...
def clone_repos(base_dwm=False, verbose=False):
    ...
    if base_dwm:
        git_repos = [
            "https://git.suckless.org/dwm", "https://git.suckless.org/slstatus", 
            "https://git.suckless.org/st", "https://git.suckless.org/dmenu"
        ]
    else:
        git_repos = [
            "https://github.com/N0pulse/dwm", "https://github.com/N0pulse/slstatus", 
    ...
    for link in git_repos:
        git_link = [link]
        try:
            p2 = subprocess.run(["git", "clone"] + git_link, cwd=f"{pathlib.Path.home()}/Suckless")
        except (subprocess.SubprocessError, BaseException, KeyboardInterrupt) as error:
            print(error)
            sys.exit(1337)
        if p2.returncode != 0:
...
def compile(verbose=False):
    ...
    for dir in directories:
        try:
            p1 = subprocess.run(["sudo", "make", "clean", "install"], cwd=f"{pathlib.Path.home()}/Suckless/{dir}")
        except (subprocess.SubprocessError, BaseException, KeyboardInterrupt) as error:
            print(error)
            sys.exit(1337)
        if p1.returncode != 0:
